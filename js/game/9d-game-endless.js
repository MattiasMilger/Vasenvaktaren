...
Game.prototype.challengeEndlessTower = function() {
    ...
    ui.showDialogue(
        'Endless Tower',
        `<p>The Endless Tower stretches infinitely into the void, a test of endurance and strength. Every ${interval} floors, Idunn replenishes your team - healing ${healPercent}% health, cleansing ${cleanseStages} debuff stage of each attribute, and renewing passives.</p>
...
Game.prototype.applyIdunnApples = function() {
    ...
    gameState.party.forEach(v => {
        ...
        attrs.forEach(attr => {
            if (v.attributeStages[attr] < 0) {
                ...
                const stageWord = cleanse === 1 ? 'stage' : 'stages';
                ui.addCombatLog(`${displayName}'s ${attr} was cleansed by ${cleanse} ${stageWord}!`, 'buff');
            }
        });

        // Heal
        const healAmount = Math.min(
            Math.floor(v.maxHealth * healPercent),
            v.maxHealth - v.currentHealth
        );
    ...
    ui.addCombatLog('Passives and runes have been renewed.', 'heal');
};

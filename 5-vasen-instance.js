...
// Create random guardian team for endless tower
function createRandomGuardianTeam(level) {
    const team = [];
    const usedRunes = new Set();
    const usedSpecies = new Set();
    
    // Select 3 random Väsen
    const availableSpecies = VASEN_LIST.filter(name => !isMythical(name));
    let mythicalAllowed = true;
    
    for (let i = 0; i < 3; i++) {
        let pool = availableSpecies.filter(name => !usedSpecies.has(name));
        
        // Allow one mythical
        if (mythicalAllowed && Math.random() < 0.3) {
            const mythicals = getMythicalVasen().filter(name => !usedSpecies.has(name));
        ...
        if (pool.length === 0) pool = VASEN_LIST.filter(name => !usedSpecies.has(name));
        ...
        // Random runes (no duplicates in team)
        const numRunes = level >= 30 ? 2 : 1;
        const runes = [];
        const availableRunes = RUNE_LIST.filter(r => !usedRunes.has(r));
        
        for (let j = 0; j < numRunes && availableRunes.length > 0; j++) {
            const index = Math.floor(Math.random() * availableRunes.length);
            const rune = availableRunes[index];
            runes.push(rune);
            usedRunes.add(rune);
            availableRunes.splice(index, 1);
        }
        
        const vasen = new VasenInstance(speciesName, level, temperament, runes);
        if (vasen.hasRune('URUZ')) {
            vasen.maxMegin = vasen.calculateMaxMegin();
            vasen.currentMegin = vasen.maxMegin;
        }
